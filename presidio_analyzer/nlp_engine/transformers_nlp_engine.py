...
class TransformersNlpEngine(SpacyNlpEngine):
    ...
    def load(self) -> None:
        ...
        for model in self.models:
            ...
            selected_model = AutoModelForTokenClassification.from_pretrained(transformers_model)
            tokenizer = AutoTokenizer.from_pretrained(transformers_model)
            ner = pipeline('ner', model=selected_model, tokenizer=tokenizer, aggregation_strategy="first")
            self.transformers_model[model["lang_code"]] = ner
    ...
    def process_text(self, text: str, language: str) -> NlpArtifacts:
        """Execute the Transformers pipeline on the given text and language."""
        if not self.nlp:
            raise ValueError("Transformers engine is not loaded. Consider calling .load()")
        predictions = self.transformers_model[language](text)
        return self._doc_to_nlp_artifact(predictions, language)
    
    def _doc_to_nlp_artifact(self, doc: List[Dict], language: str) -> NlpArtifacts:
        spans = []
        for pred in doc:
            span = doc.char_span(
                pred["start"], 
